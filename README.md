...
## Практическое задание 23 — Контейнеризация с Docker (Nginx + Backend)

развёртывание микросервисного приложения с использованием Docker и Docker Compose. Продолжение практики 22, где каждый компонент системы теперь изолирован в отдельном контейнере.

- **Контейнеризация:** создание Dockerfile для Node.js сервисов с оптимизацией слоёв (кэширование `npm install`)
- **Оркестрация:** использование Docker Compose для управления стеком из Nginx и нескольких инстансов бэкенда
- **Сети Docker:** организация взаимодействия между контейнерами по именам сервисов в изолированной сети
- **Балансировка нагрузки:** Nginx как точка входа, распределяющая трафик между бэкенд-серверами
- **Отказоустойчивость:** настройка `max_fails` и `fail_timeout` в Nginx для автоматического исключения упавших контейнеров из ротации

**Технологии:** Docker, Docker Compose, Nginx, Node.js (Express), WSL 2
